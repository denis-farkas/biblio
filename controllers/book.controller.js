...
const createBook = async (req, res) => {
  try {
    const { title, autor, resume, published_at, cover, genre, verified } =
      req.body;
    const response = await bookDB.createBook(
      title,
      autor,
      resume,
      published_at,
    ...
    return res
      .status(201)
      .json({
        message: "Votre livre a été ajouté avec succès",
        id_books: response.result.insertId,
      });
  } catch (error) {
...
const updateBook = async (req, res) => {
  try {
    const { title, autor, resume, published_at, cover, genre, verified } =
      req.body;
    const id_books = req.params.id_books;
    const response = await bookDB.updateBook(
      title,
      autor,
      resume,
      published_at,
    ...
    return res
      .status(201)
      .json({ message: `Le livre ${title} a été modifié avec succès` });
  } catch (error) {
    console.error("Controller error", error);
    return res.status(500).json({ message: error.message });
  }
